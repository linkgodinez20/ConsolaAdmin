...
//);
    ...
    //Alert functions with message
	function successMessage(titulo, mensaje) {
	    if (document.getElementById('cbTodos').checked == true)
	        eliminaNotificaciones();
	    else
	        eliminaArea("logSuccess");
	...
	function infoMessage(titulo, mensaje) {
	    if (document.getElementById('cbTodos').checked == true)
	        eliminaNotificaciones();
	    else
	        eliminaArea("logInfo");
	...
	function warningMessage(titulo, mensaje) {
	    if (document.getElementById('cbTodos').checked == true)
	        eliminaNotificaciones();
	    else
	        eliminaArea("logWarning");
	...
	function dangerMessage(titulo, mensaje) {
	    if (document.getElementById('cbTodos').checked == true)
	        eliminaNotificaciones();
	    else
	        eliminaArea("logDanger");
	...
	function BootMessage(titulo, mensaje, aType, borrar) {
	    //1 success 2 info 3 warning 4 danger
        //1 green   2 blue 3 yellow  4 red 
	    //borrar 1 todas las notificaciones
	    //borrar 0 toda su area
	    ...
	    if (borrar == 1)
	        ...
	        eliminaArea("logDanger");
	    var html;
	    html = "<div class='alert alert-" + aType + "' style='margin-bottom: 5px;'>" +
		"<button type='button' class='close' data-dismiss='alert'>×</button>" +
		"<strong>" + titulo + " </strong> " + mensaje + "</div>";
	    $("#logDanger").append(html);
	}
